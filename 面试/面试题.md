...
2. 实现 Runnable 接口
3. 实现 Callable 接口
4. 通过线程池创建线程 (new ThreadPoolExecutor(), Executors.newFixedThreadPool 等等)
...
> 自动装配bean
3.1 (prepareRefresh)环境准备。准备servlet相关的环境environment。对servletContextInitParams和servletConfigInitParams赋值。检验是否有必填的环境变量
3.2 3.3(obtainFreshBeanFactory、prepareBeanFactory)准备类加载器、表达式解析器、配置文件处理器、bean后置处理器。注册特殊bean和系统级bean，将他们放入特殊对象池和单例池中。
3.4 (postProcessBeanFactory)对beanFactory额外设置和修改，定义request session的相关作用域，注册与servlet相关的一些特殊bean（包括ServletRequest、HttpSession）
3.5 (invokeBeanFactoryPostProcessors)执行beanFactory后置处理器，主要是配置处理器 ConfigurationClassPostProcessor。通过他加载所有@Configuration配置类。
同时检索自定义的bean扫描路径。扫描每个类，将扫描出来的bean定义放在bean定义池beanDefinitionMap中
3.6 (registerBeanPostProcessors) 检索bean后置处理器，放在后置处理器池中
3.7 3.8 (initMessageSource、initApplicationEventMulticaster)从单例池中获取两个bean放在ApplicationContext中。（一个用于国际化，多语言切换，另一个用于自定义广播事件）
3.9 (onRefresh)启动web服务器
3.10 (registerListeners)bean中查找监听器bean，注册到消息广播器中
3.11 (finishBeanFactorylnitialization)通过finishBeanFactoryInitialization生产所有的bean。整体分为构造对象、填充属性、初始化实例、注册销毁。bean生成后放在单例池中。
3.12 (finishRefresh)构造注册生命周期管理器lifecycleprocessor，调用bean的start方法，发布容器刷新完成事件
