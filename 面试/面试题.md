...
## 分布式事务。微服务多模块怎么保证事务一致性
问题1
模块a执行update1，调用模块b方法，执行update2，dubbo的rpc调用，但是b的方法时间长，a等待超时报错回滚，(或者是执行update2报错回滚)
，但是b最终执行成功了。数据不一致。
问题2
模块a执行updatea1，调用模块b方法，执行updateb1，成功，模块a再执行updatea2,报错，a会回滚，但是b不会回滚，因为b事务已经提交了，本地事务不会跨服务

解决：
方案一 分布式事务
Seata AT 模式（还有xa等别的模式），MySQL+SpringBoot 前提，
Seata 是目前 Dubbo + Spring Cloud 最常用的分布式事务方案。
优点：开发方便，加注解即可，立即一致性
缺点：性能会下降（有锁），不能超过2个服务
适用场景：适合订单/支付等关键操作，实际主流做法，因为性能差，所以只有简单的内部系统图方便才用at
...
缺点:
1. 实现复杂。
2. 数据是最终一致，不是立即一致 业务刚好要求 强一致性（例如转账、支付、扣库存业务），MQ 不一定适合。
适合场景：首选 ，只要能接受异步，都用这个，性能最好。发放积分、发短信，发优惠券 等 。

方案三 tcc机制
tcc机制，引入Seata tcc依赖，其实是冻结余额，并不是先扣再加
调用b的方法后，
失败 → 调 B.cancel
成功 → 调 B.confirm
优点：性能较高，强一致，
缺点：开发成本高，需要写try、cancel、confirm的逻辑，保证幂等，表要加冻结字段，业务要有兜底机制，一直重试报错要有监控。
要单独起seata tcc server服务、建特定表
适用场景：转账，银行金融金额相关

方案四， Seata saga 补偿 + mq
先扣，失败再加回去
优点：性能高，最终一致，实现简单，不需要写cancel，confirm，只要写补偿逻辑即可
缺点：是先扣减，异常再加回去，业务要能接受（银行转账类似就不能接受）。
也要启动 seata server。写 正逻辑和补偿逻辑，定义状态机的json（相比tcc少写一个方法，表不用额外建字段）
建特定表
适用场景：用户下单 -> 预订机票 （但是具体也看业务，这里会出现下单成功，但是回头补偿，用户可能就不知道了，这种可能也要用tcc）   
下单-扣减库存
录入人员信息-开通账号-分配电脑。 发货流程  。只要是要求实时一致性，但是能接受先减再加的，都行

还有几种不合适的方案
1. 调大超时时间，治标不治本
2. 把逻辑代码移到同一个模块下，有时候业务不合适
3. 手动补偿，需要解决重复cancel、cancel失败等问题。
4. 二阶段、三阶段提交。 2PC 和 3PC 都不能保证数据100%一致，因此一般都需要有定时扫描补偿机制。
5. 本地消息表
6. 最大努力通知
...
目前市面大厂，主流做法是。
能异步就mq，比如发放积分。
不能异步、需要强一致的，比如扣库存，用saga
不能异步、需要强一致的，且不能接受先减再加的，用tcc

问题：
1. 为什么方法积分不用tcc或者saga
因为tcc、saga实现复杂，mq简单，mq性能高，能解耦，业务能接受异步
2. 为什么下单扣库存不用mq、tcc
需要强一致，排除mq（不能保证实时、顺序性）。tcc性能差，实现复杂。能接受先减再加
3. 转账为什么不用mq或者saga
因为强一致，排除mq，因为转账不能接受先减再加，所以不用saga
...
## tcc框架选型
1. Seata tcc （推荐）
开源，最活跃、最主流，支持多种模式：AT、TCC、Saga、XA，TCC 支持完善，生态好
Spring Boot / Dubbo / Spring Cloud 友好
使用场景：Java 项目做分布式事务首选。
2. Hmily（Java）
专注于 TCC（也支持 Saga）
使用场景：想要一个专注于 TCC 的框架时选择。
3. ByteTCC
字节跳动内部使用后开源 比较轻量，专门做 TCC，没有 Seata 那么大的生态
使用场景：轻量化需求，或对性能敏感的系统。
4. tcc-transaction
经典的老牌 TCC 框架，早期很流行
但现在 基本不维护了
Spring Boot 的适配性一般
...
## tcc设计是confirm和cancel一定要成功。不成功怎么办
try阶段做复杂业务操作。confirm和cancel只做状态修改update，所以一定能成功。万一因为网络或其他问题不成功，seata tcc会一直失败重试。
所以confirm和cancel要保证幂等，能一直重试。重试阶段会打报错日志，但是前端在try成功时已经返回成功，所以前端无法感知。
如果confirm、cancel一直报错，会造成实际没成功，我们不知道，所以需要做监控，监控Confirm failed ，Cancel failed 关键词，然后发钉钉。
或则监控 Seata 表，global_table、branch_table 查看长时间未完成任务
```
select *
from global_table
where status in (1, 2) -- 正在提交 / 回滚
and gmt_modified < now() - interval 10 minute;
```
还要有兜底机制

## saga框架选型
1. Seata Saga（最推荐）
2. Axon Framework（Saga 专家）
3. Camunda / Flowable（工作流 + Saga）
4. Apache ServiceComb Saga
5. Hmily Saga

## mq+dlq
主流做法：
mq手动模式，然后消费者try catch， 利用spring的 retry 重试3次后，进入catch，
catch里打日志，然后channel.basicNack(tag, false, false); 不重新入队，进入dlq。
### 问题
1. 为什么不选择自动模式
答: 核心业务用手动，可以catch里写补偿操作，ack，nack更灵活能自定义，场景比如：支付、库存
    非核心业务用自动，写起来简单，不需要写try catch
自动模式异常后会打日志，自动reject

2. 手动模式一定要catch吗
答：一定要。如果不catch，不nack，那么消息重试后仍然报错后，线程不再处理。mq不再重发，消息处于unack状态，等下次服务重启，消费者又能消费到消息。
catch里要写 error日志和 channel.basicNack(message.getMessageProperties().getDeliveryTag(), false, false);
第三个参数要false，会进入dlq，没有dlq就抛弃了。如果是true，那么消息马上放回mq，马上要消息，会一直循环。

3. spring retry是什么，是mq的重试吗
答：spring retry是spring的框架重试，和mq没有关系。可以yaml配置.也可以代码里配置自定义工厂，
但是要注意，代码里配置自定义mq工厂
记得要加 
```
.recoverer((msg, ex) -> { log.error("mq消息最终失败，进入recoverer:", ex);  })
或者
...
### dlq用法
配置类中，普通队列绑定dlq，消息进入dlq后，处理方案：
1. 放在dlq中，等人工手动解决
2. 写消费者，消费dlq消息，逻辑处理，可以保存在数据库中，可以发告警邮件短信
3. 重新放回原队列，不推荐，容易导致消息循环。


### 限制用户一天内只能点击2次
场景
因为实名认证要钱，成本高，所以要限制次数，每个用户24h内最多只能认证2次。


实现方案：
1. mysql记录认证记录。redis对用户id加锁，防止并发，多次同时点击
   优点，持久可查，可做风控日志，精度高，缺点，并发高时，需要redis加锁限流

2. redis记录认证记录，（24h过期 ）redis对用户id加锁
   优点：实现简单，容易理解。缺点：性能略差（因为涉及 Redis 多次访问 + 等锁时间），但一般用户操作频率不高，完全可以接受。

3. redis记录认证记录，用lua脚本一次查询更新。就不用加锁了
   优点 所有逻辑在 Redis 内原子执行；并发再高也不会错；无需外部锁；性能极高。
   缺点，写起来复杂， 还要写lua脚本，不记日志

4. Redisson 限流器
   RRateLimiter limiter = redissonClient.getRateLimiter("realname:limit:" + userId);
   limiter.trySetRate(RateType.PER_CLIENT, 2, 24, RateIntervalUnit.HOURS);
   if (!limiter.tryAcquire()) {
   throw new BizException("24小时内最多认证两次");
   }

内部使用 Lua 脚本 + Redis 原子操作
RRateLimiter 采用 令牌桶算法：
你设置一个桶容量（比如 2 次）
每当用户请求时尝试从桶里取一个令牌；
桶空了就阻塞/失败；
桶中的令牌按你设置的时间速率恢复。
你可以设置「固定窗口」或「时间间隔补充」等模式。

优点，实现简单，性能高，没用锁，缺点，固定窗口算法，没有自己写lua脚本灵活，只限速不记日志
缺点：更新配置时，如果key不变，没法立即生效，因为设置限流器只有第一次设置是成功的，后面不会变

总结：
需要记录认证记录的。用mysql存+redis锁
高性能，则用 redis存+lua脚本
低频高价值操作：Redisson 限流器    实名认证、领取优惠券，短信发送
其他：redis存+redis锁

### 延迟双删
延迟双删的笔记整理下，为啥第二次删除要等一会，就算延迟，极端情况下仍然会有问题
延迟删除 也会报错
